...
To make this incremental increase in representational capacity explicit,
...
parameter count (249,331 parameters) and spatial flexibility, but 
...
feature extraction under constrained data regimes, by achieving a 
favorable capacity–efficiency trade-off with 45,523 trainable parameters. 
...
Layer normalization is systematically employed to stabilize optimization 
under small-batch training conditions, while residual connections facilitate 
gradient propagation through deeper stacks of layers.
...
First, depthwise separable convolutions are employed in Models A and C. In Model A, 
Keras \texttt{SeparableConv2D} layers are used to reduce parameter count and computational cost 
while preserving spatial modeling capacity. In Model C, separable convolutions are 
implemented explicitly via depthwise and pointwise operations and are combined with 
conditional residual connections. These residual shortcuts are applied only when spatial 
resolution and channel dimensionality match, improving gradient flow without introducing 
...
Third, spatial downsampling strategies differ across architectures. Models A and B rely on 
MaxPooling layers for progressive spatial reduction, whereas Model C performs downsampling 
through strided convolutions (both in the initial stem and in depthwise blocks). This design 
allows Model C to learn feature extraction and resolution reduction jointly, preserving richer 
intermediate representations.

At the classification stage, Models A and C adopt Global Average Pooling, which enforces 
translation invariance and reduces overfitting by limiting the number of trainable parameters 
in the classifier. In contrast, Model B uses flattening, resulting in a higher-capacity fully 
connected head. Dropout regularization is applied in Models A and C before the final classifier, 
whereas Model B relies solely on architectural simplicity for regularization.
