...
are also compared in terms of trainable parameter count.
...
mainly due to the \texttt{Flatten}+\texttt{Dense} stage operating on high-dimensional 
feature maps.
...
All architectures are optimized with the Adam optimizer. Training is regularized through early stopping
monitored on validation accuracy and by an adaptive learning-rate schedule (ReduceLROnPlateau),
which lowers the learning rate when validation performance stagnates. These mechanisms improve
training stability and reduce unnecessary computation once progress plateaus.

To ensure a fair comparison across architectures, all models follow the same experimental protocol,
including the stratified split and fixed seed (Sec.~1.2) and the tuning procedure used to select
architecture-specific learning rates and related settings (Sec.~2.4). Data augmentation is applied to the
training set only, while the validation set is kept unaugmented to provide an unbiased estimate of
