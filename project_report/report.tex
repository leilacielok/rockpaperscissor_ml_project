...
For comparability across architectures, all models are trained under the same experimental protocol (stratified split and fixed seed as in Sec. 1.2) and the same augmentation policy. Data augmentation is applied only to the training set, while the validation set is kept unaugmented to provide an unbiased estimate of in-distribution performance.
...
and provide insight into how each architecture learns over time, whether
...
shown in Figure~X (models A–C), ordered by decreasing prediction 
confidence.
...
More broadly, the comparison across models highlights the central role 
of architectural capacity in determining whether a model underfits, 
generalizes effectively, or risks overfitting.
...
The systematic class collapse observed for models A and B, and its 
mitigation in model C, are clearly illustrated by the confusion matrices 
reported in Appendix~C (Figure~\ref{fig:app_custom_confusion_combined}).
...
conditions. This behaviour is further characterized by the mean predicted 
class priors reported in Appendix~C (Table~\ref{tab:app_custom_pred_priors}).
