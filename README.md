...
--- 
## 📁 Reports & Outputs: Which Script Generates What
...
### 2️⃣ python main.py with Tuning Enabled — Final Best Model
...
To inspect a saved model (architecture, parameters, validation performance):
...
* Recompute validation accuracy
* Save a confusion matrix and classification report under reports/
---
✨ **Notes**

- Models are reproducible with ```config.SEED```. 
- Works on CPU, faster on GPU. 
- Architecture C integrates residual connections, dropout, and label smoothing to improve generalization.
