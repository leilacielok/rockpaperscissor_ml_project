...
Kaggle repository containing images of hand gestures for the
Rock--Paper--Scissors game. It consists of a total of 2,188 RGB images,
...
External datasets are introduced only in the evaluation phase and are
not used during training or model selection.

\subsection{Image Preprocessing: Resizing and Normalization}

To ensure compatibility with convolutional neural network architectures
and to standardize the input space, all images were resized to a fixed
...
To investigate the impact of architectural complexity on image
classification performance, three convolutional neural network (CNN)
architectures of increasing expressiveness were designed and evaluated.
model A serves as a lightweight baseline, intended to capture coarse
visual patterns with a limited number of convolutional and fully
connected layers. model B increases representational capacity by
introducing more efficient convolutional operations, allowing for
improved feature extraction while maintaining a relatively compact
parameterization. model C further extends this design by incorporating
deeper convolutional blocks and architectural regularization mechanisms,
enabling the network to model more complex spatial relationships and
hierarchical features. This progressive design strategy allows for a
systematic comparison between simple and more expressive architectures
under identical training conditions.

\subsection{Architectural Choices and Design Rationale}

Each CNN architecture was constructed using standard building blocks
commonly adopted in image classification tasks. Convolutional layers
with ReLU activations were employed to extract spatial features,
followed by max-pooling layers to progressively reduce spatial
dimensionality while retaining salient information. Dropout layers were
introduced in the deeper architectures to mitigate overfitting by
discouraging co-adaptation of neurons. The final classification stage
consists of fully connected layers followed by a softmax output layer,
producing class probabilities over the three target categories
(\emph{rock}, \emph{paper}, and \emph{scissors}). The architectural
differences between models A, B, and C are motivated by the need to
balance model capacity and generalization: simpler models favour
robustness and computational efficiency, while deeper architectures aim
to improve discriminative power through richer feature representations.

\subsection{Training Setup and Optimization Strategy}

All models were trained using the same optimization framework to ensure
a fair comparison across architectures. Training was performed using the
Adam optimizer, which provides adaptive learning-rate adjustments and
has proven effective for convolutional architectures. Categorical
cross-entropy was employed as the loss function, consistent with the
multi-class nature of the classification task. Model performance was
monitored using accuracy on a held-out validation set. Early stopping
was applied based on validation performance to prevent overfitting,
while learning-rate reduction on plateau was used to improve convergence
stability. This unified training setup ensures that observed performance
differences can be attributed primarily to architectural choices rather
than optimization artifacts.

\subsection{Hyperparameter Tuning and Final Model Selection}

Hyperparameter tuning was performed on the best-performing architecture
identified in the initial comparison using a stratified hold-out
validation set. A targeted search over key hyperparameters---including
learning rate and number of training epochs---was conducted with
validation accuracy as the primary selection criterion. Early stopping
was employed to ensure efficient and automatic model selection. The
configuration achieving the best validation performance was retained and
subsequently retrained to convergence, yielding the final model used for
detailed evaluation and generalization analysis.
