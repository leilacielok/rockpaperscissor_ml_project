...
def run_tuning(
    ...
    for name in model_names:
        for lr, batch, aug in product(
            search_space["lr"], search_space["batch"], search_space["augment"]
        ):
...
def run_final_training(
    ...
    history, runtime = training.train(
        model, train_ds, val_ds, epochs=epochs, callbacks=cbs, learning_rate=lr
    )
    print(
        f"Final training time: {runtime:.1f}s | Best val_acc: {max(history.history['val_accuracy']):.4f}"
    )
