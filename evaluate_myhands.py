...
def _save_summary(
    ...
    for c in classes:
        m = per_class[c]
        lines.append(
            f"- {c}: P={m['precision']:.3f}  R={m['recall']:.3f}  F1={m['f1']:.3f}  (support={m['support']})"
        )
    lines.append("")
    ...
    if pred_prior is not None:
        lines.append(
            f"- Pred prior (mean probs pre-recalib): {np.round(pred_prior, 3)}"
        )
...
def _load_img(path, img_size, resize_mode="pad"):
    ...
    if resize_mode == "pad":
        x = tf.image.resize_with_pad(
            x, img_size, img_size, method="bilinear", antialias=True
        )
    else:
...
def _parse(path, label, img_size, n_classes, resize_mode="pad"):
    return _load_img(path, img_size, resize_mode=resize_mode), tf.one_hot(
        label, n_classes
    )
...
def load_labeled_dir(root_dir, batch_size=None, resize_mode="pad"):
    ...
    ds = (
        tf.data.Dataset.from_tensor_slices((files_tf, labels_tf))
        .map(
            lambda p, y: _parse(
                p, y, img_size, n_classes, resize_mode
            ),  # map: decode+resize+one-hot
...
def _tta_views(xb, use_rot=False):
    ...
    if use_rot:
        try:
            ...
            for deg in (-12, +12):
                views.append(
                    tfa.image.rotate(xb, np.deg2rad(deg), interpolation="bilinear")
                )
        except Exception:
            pass
    return views
...
def main(model_path: str, data_dir: str, outdir: str | None):
    ...
    if pre_scale_255:
        print(
            "⚠️  Detected model Rescaling(1/255) at input → multiply inputs ×255 before prediction."
        )
    ...
    for xb, yb in ds:
        if USE_TTA:
            p = _predict_with_tta(
                model, xb, use_rot=USE_TTA_ROT, pre_scale_255=pre_scale_255
            )
        else:
    ...
    if best_acc > id_acc + 1e-6:
        print(
            f"🔁 [auto-fix] Using class permutation {best_perm} (acc={best_acc:.3f}) "
    ...
    if RECALIB_MODE == "uniform":
        probs, w, target_prior, pred_prior = _recalibrate_probs(
            probs, y_true, "uniform", RECALIB_ALPHA
        )
        pred_prior_used, target_prior_used, weights_used = pred_prior, target_prior, w
        print(
            f"▶ Using UNIFORM recalibration (alpha={RECALIB_ALPHA}). "
            f"pred_prior={np.round(pred_prior, 3)} weights={np.round(w, 2)}"
        )
    elif RECALIB_MODE == "empirical":
        probs, w, target_prior, pred_prior = _recalibrate_probs(
            probs, y_true, "empirical", RECALIB_ALPHA
        )
        pred_prior_used, target_prior_used, weights_used = pred_prior, target_prior, w
        print(
            f"▶ Using EMPIRICAL recalibration (alpha={RECALIB_ALPHA}). "
    ...
    _save_summary(
        ...
        settings,
        used_perm,
        pred_prior=pred_prior_used,
        target_prior=target_prior_used,
    ...
    evaluation.save_report(
        report_txt, os.path.join(outdir, f"classification_report_{tag}.txt")
    )
    ...
    try:
        ...
        print(
            f"🖼️  Saved misclassified grid to {os.path.join(outdir, f'misclassified_{tag}.png')}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Where to save reports/plots. If omitted, uses "
